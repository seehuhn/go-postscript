...
func (info *decodeInfo) decodeCharString(code []byte, name string) (*Glyph, *GlyphInfo, error) {
	const maxStack = 24
	...
	isClosed := true
	rClosePath := func() {
		resG.Cmds = append(resG.Cmds, GlyphOp{Op: OpClosePath})
		isClosed = true
	}
	rMoveTo := func(dx, dy float64) {
		...
		posX += dx
		posY += dy
		resG.Cmds = append(resG.Cmds, GlyphOp{
			Op:   OpMoveTo,
	...
	opLoop:
		...
		opSwitch:
			...
			case t1hsbw:
				...
				LsbY = 0
				resI.WidthX = funit.Int16(math.Round(stack[1]))
				resI.WidthY = 0
				clearStack()
			case t1seac:
				if len(stack) < 5 {
			...
			case t1sbw:
				...
				resI.WidthX = funit.Int16(math.Round(stack[2]))
				resI.WidthY = funit.Int16(math.Round(stack[3]))
				clearStack()
			...
			case t1hstem:
				...
				resG.HStem = append(resG.HStem, a, b)
				clearStack()
			case t1hstem3:
				if len(stack) < 6 {
					return nil, nil, errIncomplete
				...
				resG.HStem = append(resG.HStem[:0], a, b, c, d, e, f)
				clearStack()
			case t1vstem:
				if len(stack) < 2 {
					return nil, nil, errIncomplete
				...
				b := a + funit.Int16(math.Round(stack[1]))
				resG.VStem = append(resG.VStem, a, b)
				clearStack()
			case t1vstem3:
				if len(stack) < 6 {
					return nil, nil, errIncomplete
				...
				f := e + funit.Int16(math.Round(stack[5]))
				resG.VStem = append(resG.VStem[:0], a, b, c, d, e, f)
				clearStack()
			case t1div:
				if len(stack) < 2 {
					return nil, nil, errIncomplete
			...
			case t1callsubr:
				...
				if idx < 0 || idx >= len(info.subrs) {
					return nil, nil, invalidSince("invalid subr index")
				}
				// fmt.Printf("callsubr(%d)\n", idx)

				cmdStack = append(cmdStack, code)
				if len(cmdStack) > 10 {
					return nil, nil, invalidSince("maximum call stack size exceeded")
				}
				code = info.subrs[idx]
			case t1callothersubr:
				...
				case 2: // flex coordinate pair (0 args)
					...
					if len(resG.Cmds) > 0 {
						...
						resG.Cmds = resG.Cmds[:len(resG.Cmds)-1]
					}
			...
			case t1pop:
				if len(postscriptStack) < 1 {
					return nil, nil, invalidSince("postscript interpreter operand stack underflow")
				}
				// fmt.Println("pop")
			...
			default:
				return nil, nil, invalidSince(fmt.Sprintf("invalid type 1 opcode %d", op))
			}
		}
	}
	if !isClosed {
